...
You will have been given one or more links to the location of C++ compiled libraries (windows DLLs), and R and/or Python packages. You should have access to at least a file `libs.7z` and R/Python packages, in files `R_pkgs.7z` and `python.7z`, or subfolders already extracted.

Other archives files (such as include.7z) may be present and used only for development. They are not necessary for most users.

# Installing shared (dynamic) or feasible librarie
...
This section is valid for developers who have access to and checked out source code. All other users can ignore it.
...
We recommend developers install the packages `nbstripout` and `jupytext` to manage python notebooks.

## Matlab functions

Matlab bindings are available and used by some users. Their installation is not documented here, but may be in the future.
