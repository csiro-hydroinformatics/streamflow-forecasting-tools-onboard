...
You will have been given one or more links to the location of C++ compiled libraries (windows DLLs), and R and/or Python packages. You should have access to at least:

* libs_third_party.7z
* libs.7z

and R/Python packages, in files `R_pkgs.7z` and `python.7z`, or subfolders already extracted.

Other archives files (such as include.7z) are used for development and not necessary for most users.
...
There are various options to set up a suotable python environment to run. This document outlines using `conda`. If you do not have any preexisting environments, we recommend you use [mambaforge](https://github.com/conda-forge/miniforge#mambaforge), but you may have already it installed on your Windows box by e.g. your IT department installation management software.
...
Then, after having activated your conda environment if need be:
